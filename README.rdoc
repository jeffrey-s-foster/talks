...
* sqlite3 db/development.sqlite3
  > update users set perm_site_admin = "1" where id = "1";
* Edit app/controllers/application_controller#generate_ical so that the two occurrences of "US/Eastern" are in the appropriate time zone.
...
* beginning of day on upcoming talks?  In my opinion, the talks on a
  given day should stay up for that day, and then the whole day moved
  to "past talks" when the day is over.  Maybe it's just me, but I
  find it both unsettling and annoying not to see the whole day's list
  of talks on one display together.
...
* make it impossible to post talk not on at least one list
