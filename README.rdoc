...
* Make the date come first (before the talk title, even) and more
  prominent; people will tend to look for things that are coming up on
  a certain date. Also when this gets into full swing, there'll be
  days with more than one talk and it'll help to see at a glance what
  one's choices are for a given date.
...
* Include (when appropriate) the *kind* of talk: general, special
  series, etc.
...
* Questions/Suggestion: when I subscribe to RSS Feeds (via Apple
  Mail), the individual feeds do not give the speaker, date and time
  of the talk until I click on "Read More." Is this the way you
  intended it or is it a product of my RSS Reader (is there a better
  way to read RSS)? If not, perhaps it might be worth adding this
  information into the feed body for ease of browsing.

* cron job not working

* link to it on the dept home page

* Can you provide a RSS feed for all upcoming talks?  It's great
  that feeds are available by list, but I'm sure that some people
  would be content to take a single feed that encompasses all publicly
  listed talks. Also ical feed

* Individual feed items do not presently include the date, time or
  location information for the talk.  Also, including links to the
  lists that the talk is associated with at the bottom of the
  abstract, like the HTML display does, would probably be helpful, as
  well.
